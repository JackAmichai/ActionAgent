 ...
 */
export async function getMeetingTranscript(meetingId: string, userId: string): Promise<string> {
 ...
 */
export async function resolveMeetingId(input: string, userId: string): Promise<string | null> {
 ...
 */
export async function hasTranscript(meetingId: string, userId: string): Promise<boolean> {
  try {
    const transcripts = await graphClient
      .api(`/users/${userId}/onlineMeetings/${meetingId}/transcripts`)
      .top(1)
