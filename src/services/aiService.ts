 ...
 */
const ActionItemZodSchema = z.object({
  title: z.string().max(255),
  assignedTo: z.string().optional().default('Unassigned'),
  type: z.enum(['Task', 'Bug', 'User Story']).default('Task'),
  priority: z.enum(['High', 'Medium', 'Low']).default('Medium'),
  description: z.string().optional(),
  deadline: z.string().optional(),
});

const AIResponseZodSchema = z.object({
  actionItems: z.array(ActionItemZodSchema),
...
2. Determine who is responsible for each task
3. Classify the type of work (Bug, Task, User Story)
4. Assess priority (High, Medium, Low)
5. Assess the overall sentiment of the meeting (Positive, Neutral, Negative)

STRICT RULES:
- ONLY extract technical/engineering tasks
- IGNORE small talk
...
Output ONLY valid JSON with this exact structure:
{
  ...
  "summary": "Brief 2-3 sentence summary of technical decisions",
  "sentiment": "Neutral"
}

If no technical action items are found, return empty array.`;
 ...
 */
export async function extractActionItems(transcriptText: string): Promise<ActionItemsResponse> {
...
async function processLargeTranscript(
  ...
  const consolidatedSummary =
    summaries.length > 0 ? await generateConsolidatedSummary(summaries) : undefined;

  return {
    actionItems: dedupedItems,
    summary: consolidatedSummary,
    sentiment: 'Neutral', // Default for merged chunks
  };
}
...
async function callOpenAI(transcriptText: string): Promise<string> {
  ...
  try {
    ...
    if (result.usage) {
      telemetry.trackMetric('AI.Tokens.Prompt', result.usage.prompt_tokens, 'count');
      telemetry.trackMetric('AI.Tokens.Completion', result.usage.completion_tokens, 'count');
      telemetry.trackMetric('AI.Tokens.Total', result.usage.total_tokens, 'count');
    }
...
function parseAndValidateResponse(
  ...
  try {
    ...
    const validationResult = AIResponseZodSchema.safeParse(parsed);

    if (validationResult.success) {
      return {
        actionItems: validationResult.data.actionItems.map(item => ({
          ...item,
          assignedTo: item.assignedTo || 'Unassigned',
          // Explicit cast because zod defaults/enums match the type but TS wants confirmation
          type: item.type as 'Task' | 'Bug' | 'User Story',
          priority: item.priority as 'High' | 'Medium' | 'Low',
        })),
        summary: validationResult.data.summary,
        sentiment: validationResult.data.sentiment as 'Positive' | 'Neutral' | 'Negative' | undefined,
      };
    } else {
      // Fallback for older formats or slight deviations
      telemetry.warn('Zod validation failed, attempting manual fallback', { error: validationResult.error });
      return manualFallback(parsed);
    }
  } catch (error) {
    telemetry.error('Failed to parse AI response', error as Error, { content });
    throw new ActionAgentError('Failed to parse AI response as JSON', context, {
      cause: error as Error,
    });
  }
}

function manualFallback(parsed: any): ActionItemsResponse {
     let actionItems: any[] = [];
     ...
     return {
       actionItems: actionItems.map((item) => ({
         title: sanitizeTitle(item.title || 'Untitled'),
         assignedTo: item.assignedTo || 'Unassigned',
...
function validateWorkItemType(type: string): 'Task' | 'Bug' | 'User Story' {
  ...
  if (normalized === 'user story' || normalized === 'story' || normalized === 'feature') {
    return 'User Story';
  }
  return 'Task';
}
 ...
 */
async function generateConsolidatedSummary(summaries: string[]): Promise<string> {
